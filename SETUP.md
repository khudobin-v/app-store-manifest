...
4. **Клиент магазина**: в [`store-client/gradle.properties`](store-client/gradle.properties)
   ...
   репозитория — это значение по умолчанию.

   Можно и не менять: в магазине есть экран настройки витрины (шестерёнка), где
   достаточно ввести GitHub-логин — приложение само найдёт репозиторий с
   `apps.json` и запомнит адрес.

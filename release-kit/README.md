...
Скрипт не знает вашего приложения, поэтому кадры снимает вслепую: запуск, две
прокрутки и свайп вбок. Перед каждым снимком проверяется, что приложение всё
ещё в фокусе — иначе в кадр попал бы чужой экран. Одинаковые кадры
выбрасываются: у экрана без прокрутки останется один снимок, а не пять копий.
